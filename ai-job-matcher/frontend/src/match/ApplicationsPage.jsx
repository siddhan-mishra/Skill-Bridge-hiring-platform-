...
export default function ApplicationsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await axios.get(`${API_BASE}/api/applications/my`, { headers: { Authorization: `Bearer ${token}` } });
        setApps(res.data);
  ...
  const inp = { background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: '7px', padding: '0.4rem 0.7rem', fontSize: '0.85rem', outline: 'none', fontFamily: 'inherit' };
  ...
  return (
    <div style={{ maxWidth: 820, width: '100%' }}>
      ...
      {filtered.length === 0 ? (
        <div className="card"><p style={{ color: '#6b7280' }}>No applications in this category. <Link to="/jobs" style={{ color: '#6366f1' }}>Browse jobs →</Link></p></div>
      ) : (
            ...
            return (
              <div key={app._id} style={{ background: '#0a0f1e', border: '1px solid #1f2937', borderRadius: '10px', padding: '1rem 1.25rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
                  <div>
                    <Link to={`/jobs/${app.job?._id}`} style={{ fontWeight: 700, color: '#e5e7eb', fontSize: '0.97rem' }}>{app.job?.title || 'Job removed'}</Link>
                    <div style={{ color: '#6b7280', fontSize: '0.84rem', marginTop: '0.15rem' }}>
                      {app.job?.company}{app.job?.location ? ` · ${app.job.location}` : ''}
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.3rem' }}>
                    <span style={{ padding: '0.2rem 0.75rem', borderRadius: '999px', fontSize: '0.78rem', fontWeight: 600, background: sm.bg, color: sm.color }}>{sm.emoji} {sm.label}</span>
                    <span style={{ fontSize: '0.72rem', color: '#4b5563' }}>Applied {new Date(app.createdAt).toLocaleDateString()}</span>
                  </div>
                </div>
                {app.coverNote && (
