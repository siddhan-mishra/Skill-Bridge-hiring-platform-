...
import { getAllRecruiterApps, updateAppStatus } from '../api/jobApi';
...
const STATUS_OPTS = ['all', 'pending', 'reviewed', 'shortlisted', 'rejected', 'hired'];
...
export default function AllApplicationsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getAllRecruiterApps();
        setApps(res.data);
      } catch (err) {
  ...
  const jobTitles = useMemo(() => {
    ...
    apps.forEach(a => { if (a.job?._id) titles.set(a.job._id, a.job.title || 'Untitled'); });
  ...
  const handleUpdateStatus = async (appId, newStatus) => {
    ...
    try {
      const res = await updateAppStatus(appId, { status: newStatus });
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.75rem' }}>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 700, color: '#e5e7eb' }}>{app.applicant?.name || 'Unknown Candidate'}</span>
                      {score !== null && (
                        <span style={{ fontWeight: 800, color: scoreColor(score), fontSize: '0.85rem' }}>{score}% match</span>
                      )}
                    ...
                    <div style={{ color: '#6b7280', fontSize: '0.82rem', marginTop: '0.2rem' }}>{app.applicant?.email}</div>
                  ...
                  <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', alignItems: 'center', flexShrink: 0 }}>
                    ...
                    {!['shortlisted','hired','rejected'].includes(app.status) && (
                      <button onClick={() => handleUpdateStatus(app._id, 'shortlisted')} disabled={updating[app._id]}
                    ...
                    {app.status === 'shortlisted' && (
                      <button onClick={() => handleUpdateStatus(app._id, 'hired')} disabled={updating[app._id]}
                    ...
                    {!['rejected','hired'].includes(app.status) && (
                      <button onClick={() => handleUpdateStatus(app._id, 'rejected')} disabled={updating[app._id]}
